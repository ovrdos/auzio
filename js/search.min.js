...
var getNextSongAuto = function(e) {
    if (LOOP_ON) {
        currentDuration = front_player.getDuration();replaySong();
    } else {
        getNextSong(e);
    }
}
...
function returnDuration(e) {
    ...
    if ("ERROR" != t) {
        ...
        o = 1e3 * o - 1700, o = Math.min(36e4, o), vtime.stop(), vtime = new Timer(function() {
            currentDuration = o, getNextSongAuto(n)
        }, o)
    }
}
...
var loopSong = function(e) {
    if (LOOP_ON) {
        $("img.loop").attr("src", "media/svg/rewind.svg");
        LOOP_ON = false;
    } else {
        $("img.loop").attr("src", "media/svg/rewind.svg");
        LOOP_ON = true;
...
var front_player, BASE_FINDERS = " lyrics -movie -album -kids -kidzbop -minecraft -pranking -karaoke -cover -parody -reaction -review -remix -live",
    PRE = '<span class="pre status">Now playing...</span><br>',
    POST = '<span class="pre"><br>' +
    '<img class="loop" src="media/svg/rewind.svg" onclick="replaySong()"/>' +
    '<img class="pause" src="media/svg/pause.svg" onclick="pauseSong()"/>' +
    '<img class="play" src="media/svg/play.svg" onclick="playSong()"/>' +
    '<img class="next" src="media/svg/next.svg" onclick="getNextSong(currentSong)"/>' +
    '<img class="share" src="media/svg/share.svg" onclick="shareSong(currentSong)"/></span><br>',
...
successCallback = function(e) {
    if (e) {
        ...
        o.length > 0 && (console.log("ALREADY PLAYED: " + t), t = e.items[1].snippet.title, n = e.items[1].id.videoId), t = t.replace(/with lyrics/gi, ""), t = t.replace(/\(lyrics\)/gi, ""), t = t.replace(/lyrics/gi, ""), t = t.replace(/lyric/gi, ""), t = t.replace(/w\//gi, ""),
        t = t.replace(/\+/gi, " "), t = t.replace(/\[/gi, ""), t = t.replace(/audio/gi, ""), t = t.replace(/video/gi, ""), t = t.replace(/\)/gi, ""), t = t.replace(/\(/gi, ""), t = t.replace(/official/gi, ""), t = t.replace(/]/gi, ""), t = t.replace(/wmv/gi, ""), t = t.replace(/mp3/gi, ""),
        t = t.replace(/\./gi, ""), t = t.trim(), $("div#search_result").html(PRE + '<span id="srtitle">' + t + "</span>" + POST), front_player.loadVideoById(n), front_player.setVolume(80), front_player.unMute(), currentSong = n, videoHistory.push({
            index: n,
            title: t
...
document.addEventListener('keypress', function(event) {
    ...
    "" !== currentSong && ((13 === e.keyCode || 13 === e.charCode) && getNextSong(currentSong), (32 === e.keyCode || 32 === e.charCode) &&
    (1 != front_player.getPlayerState() ? (isPaused = false, front_player.playVideo(), showVisual(), vtime.resume()) : (isPaused = true, front_player.pauseVideo(), hideVisual(), vtime.pause())));
});
...
var playDeepLink = function() {
    let link = document.location.hash.split("#");
    let vid = link.substring(1);
    console.log(vid);
    if (vid) {
        getSong(vid);
    }
